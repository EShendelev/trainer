package SprintFive.ThirdTheme.Ex7;
...
public class PotatoLaboratory {
    ...
    private static List<Potato> findPotatoesForExperiment(List<Potato> potatoes) {
        ...
        List<Potato> potatoArr = new ArrayList<>(potatoes);
        List<Potato> res = new ArrayList<>();
        Collections.sort(potatoArr);
        res.add(potatoArr.get(0));
        res.add(potatoArr.get(1));
        res.add(potatoArr.get(potatoArr.size()-2));
        res.add(potatoArr.get(potatoArr.size()-1));
        return res;
    }
}